...
public class TestUtils {
    ...
    public static List<String> getDatesWhenDrinkIsFruitDrinkOrSyrup(List<LunchItem> items) {
        return items.stream().filter(lunchItem -> lunchItem.getKind().equals(LunchItemKind.DRINK)).filter(lunchItem -> lunchItem.getName().contains("Sirup") || lunchItem.getName()
                .contains("Ovocný nápoj")).map(LunchItem::getDate).collect(Collectors.toList());
    }
     ...
     * a hodnotou List jídel obashující dané alergeny.
     *
     * @param items
     * @return
     */
